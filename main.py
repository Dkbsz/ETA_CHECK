...
def enter_ctds_and_get_results(driver, carrier, url, ctds, input_id, result_xpath):
    driver.get(url)
    accept_cookies(driver, carrier) # < poprzednio użyta funkcja do akceptowania ciasteczek
...
for carrier, ctds in data_dict.items():
    if carrier in carrier_info:
        ...
        if carrier == 'MAEU':
            ...
            modified_ctds = [(index, ctd[4:] if ctd.upper().startswith('MAEU') else ctd) for index, ctd in ctds]
            enter_ctds_and_get_results(driver, carrier, url, modified_ctds, input_id, result_xpath)
        else:
            enter_ctds_and_get_results(driver, carrier, url, ctds, input_id, result_xpath)
